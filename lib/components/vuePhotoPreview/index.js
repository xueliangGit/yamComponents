      ...
      methods: {
        $previewRefresh () {
          setTimeout(() => {
            ...
            // this.galleryElements = document.querySelectorAll('[preview]') || document.querySelectorAll('img[preview]')
        ...
        parseThumbnailElements (thumbElements) {
          return new Promise(resolve => {
            ...
            function getImage (index, el, rw, rh) {
              var l = new Image()
              l.src = el.getAttribute('large') ? el.getAttribute('large') : el.getAttribute('src')
              console.log('getImage', l, thumbElements.length)
        ...
        initPreview (gallerySelector) {
          ...
          for (var i = 0, l = this.galleryElements.length; i < l; i++) {
            this.galleryElements[i].setAttribute('data-pswp-uid', i + 1)
            // 兼容一下 div
            if (!this.galleryElements[i].src) {
              this.galleryElements[i].setAttribute('src', getImgFromBg(this.galleryElements[i].style.backgroundImage))
            }
            this.galleryElements[i].onclick = this.onThumbnailsClick
          }
        }
